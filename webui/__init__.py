...
from manager.ProcessManager import BixbitedManager, WalletRPCManager
...
class BaseWebUI(QMainWindow):
    def __init__(self, html, app, hub, window_size, debug=False):
        ...
        self.icon = self._getQIcon('bixbite_icon_64.png')
...
class NewWalletWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        window_size = qt_core.QSize(840, 618)
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'bixbite_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'bixbite_16x16.png'
        else:
            tray_icon = 'bixbite_32x32_ubuntu.png'
        ...
        self.bixbited_daemon_manager = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def start_deamon(self):
        #start bixbited daemon
        self.bixbited_daemon_manager = BixbitedManager(self.app.property("ResPath"), 
                                            self.app_settings.settings['daemon']['log_level'], 
                                            self.app_settings.settings['daemon']['block_sync_size'])
        
        self.bixbited_daemon_manager.start()
    ...
    def run_wallet_rpc(self, wallet_password, log_level=0):
        ...
        while True:
            ...
            bixbited_info = self.daemon_rpc_request.get_info()	
            if bixbited_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if bixbited_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(bixbited_info['height'])
            target_height = int(bixbited_info['target_height'])
        ...
        sync_status = "Disconnected" if bixbited_info['status'] != "OK" else "Synchronizing..."
        if bixbited_info['status'] == "OK" and self.current_height == self.target_height:
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def about(self):
        QMessageBox.about(self, "About", \
            u"%s <br><br>Copyright© 2018 - Bixibite Project (www.bixbite.pro)" % self.agent)
    ...
    def _handleAboutToQuit(self):
        ...
        if self.wallet_rpc_manager is not None:
            self.wallet_rpc_manager.stop()
        if self.bixbited_daemon_manager is not None:
            self.bixbited_daemon_manager.stop()
